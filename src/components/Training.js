            ...
            transition duration-200 ease-out">
                ...
                <div className="flex flex-col w-full h-24 md:h-full pl-2 overflow-hidden">
                    ...
                    <p className="flex items-center">
                        ...
                        <span className="ml-1 text-sm">{training.avg_rating.toFixed(2)}</span>
                        <span className="w-1 h-1 mx-1.5 bg-slate-600 rounded-full" />
                        <span className="text-sm font-medium text-slate-600 underline hover:no-underline">
                    ...
                    <div className="text-xs items-center font-bold px-2.5 pt-1.5 space-y-1">
                        {training.tags.map((tag, i) => (
                            <span
                                className="bg-cyan-100 text-cyan-800 font-medium mr-2 rounded overflow-hidden"
                                key={i}
                            >
                                {tag}
                            </span>
                    ...
                    <div className="flex items-center text-[.6em] pt-1 pl-2.5">
                        ...
                        <span className="text-slate-900 pl-1">{training.is_online ? "Online" : "In-person"}</span>
                        {/* little dot between */}
                        ...
                        <span className="text-slate-900 pl-1">{training.is_external ? "External" : "Internal"}</span>
                    </div>
                    {/* comment */}
                    <p className="pt-2 text-[.7rem] text-ellipsis overflow-hidden text-slate-600 line-clamp-4 pr-2">
                        ...
                        <span className="inline pl-1">"{training.top_review}"</span>
                    </p>
                </div>
            </div>
        </Link>
